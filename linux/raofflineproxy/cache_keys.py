...
def parse_game_id_from_last_played_key(cache_key: str) -> int | None:
    if not cache_key.startswith(PREFIX_LAST_PLAYED):
        return None

    value = cache_key.removeprefix(PREFIX_LAST_PLAYED)
    return int(value) if value.isdigit() else None
