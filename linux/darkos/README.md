...
- ROMs root: `/roms` (KNULLI: `/userdata/roms`); Tools entries live under `/roms/tools`, bind-mounted to `/opt/system/Tools`. On a two-card device that has run ArkOS's "Switch to SD2 for Roms", that bind comes from `/roms2/tools` and every path is rewritten to `/roms2`, while `/roms` stays mounted. Both the installer and `resolve_rom_root()` therefore identify the active card from the Tools bind source rather than from `/roms` existing.
