...
RAOfflineProxy ("the app") is a local proxy tool for Android that enables offline RetroAchievements support with supported emulators such as RetroArch, Dolphin, and PPSSPP. This privacy policy explains what data is handled by the app, how it is stored, and when it is transmitted.
